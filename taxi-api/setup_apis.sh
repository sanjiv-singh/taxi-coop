...
cp ../taxi-db/package/rds-combined-ca-bundle.pem .aws-sam/build/TCRidesFunction/
cp ../taxi-db/package/rds-combined-ca-bundle.pem .aws-sam/build/TCAcceptRideFunction/
...
QUERY_LAMBDA_ARN=$(aws lambda get-function --function-name taxidb_query --query Configuration.FunctionArn --output text)

# Deploy the lambda functions using sam deploy
sam deploy --stack-name taxi-coop --parameter-overrides SouthWest=12.8,77.5 NorthEast=13.5,78.2 DBUser=$DB_USER DBPassword=$DB_PASSWORD DBEndPoint=$DB_ENDPOINT DBSecurityGroups=$VPC_SECURITY_GROUP_IDS DBSubnets=$VPC_SUBNET_IDS RegisterLambdaArn=$REGISTER_LAMBDA_ARN QueryLambdaArn=$QUERY_LAMBDA_ARN
